 ...
 *
 * The badge shows immediately. Its recipient can hide or decline it, and an
 * admin can revoke it; see migration 008 for why the default flipped.
 */
export function grantBadge(
  ...
  // Badges are opt-out (migration 008): accepted on arrival, and the recipient
  // turns one off if they want to. Nothing writes `pending` any more.
  const state: BadgeState = "accepted";
